  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border rounded p-4 bg-gray-50">
        ...
        <div className="text-sm">Pincode: {form.pincode || "--"}</div>
        <div className="text-sm">Gender: {form.gender || "--"}</div>
        <div className="text-sm">Nationality: {form.nationality || "--"}</div>
      </div>
      ...
      <div className="border rounded p-4 bg-gray-50">
        ...
        {(form.services || []).length === 0 ? (
          <div className="text-sm text-gray-500">No services added</div>
        ) : (
      ...
      {!isEdit && (
        <div className="border rounded p-4 bg-yellow-50 text-sm">
          Final submit will set status to <b>pending</b>. Admin must approve your profile before it appears to users.
        </div>
      )}
      ...
      {isEdit && (
        <div className="flex justify-end gap-2 pt-4">
          ...
          <button onClick={onSave} className="px-4 py-2 bg-black text-white rounded">
            Save Changes
          </button>
        </div>
      )}
