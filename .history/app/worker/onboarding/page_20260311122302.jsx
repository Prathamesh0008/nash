...
export default function WorkerOnboardingPage() {
  ...
  const cardClass =
    "onboard-card space-y-2 rounded-xl border border-slate-700/70 bg-slate-900/35 p-4 backdrop-blur-sm";
  const inputClass =
    "w-full rounded border border-slate-700 bg-slate-900 p-2 transition-all duration-300 ease-out focus:-translate-y-0.5 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/30 focus:outline-none";
  ...
  return (
    <section className="onboard-shell mx-auto max-w-3xl panel space-y-4">
      ...
      <div className="onboard-card rounded border border-slate-700 bg-slate-900/40 p-3">
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          {onboardingCompletion.checklist.map((item, idx) => (
            <p
              key={item.key}
              className={`animate-in-checklist rounded px-2 py-1 text-xs ${
      ...
      <form onSubmit={submit} className="onboard-form space-y-3">
        <div className={cardClass}>
          <p className="text-sm text-slate-300">Profile Photo</p>
          <input className={inputClass} placeholder="Profile photo URL" value={form.profilePhoto} onChange={(e) => setForm({ ...form, profilePhoto: e.target.value })} />
        ...
        <div className={cardClass}>
          ...
          <input className={inputClass} placeholder="Categories comma separated" value={form.categories} onChange={(e) => setForm({ ...form, categories: e.target.value })} />
          <div className="grid gap-3 md:grid-cols-2">
            <input className={inputClass} placeholder="City" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} />
            <input className={inputClass} placeholder="Pincode" value={form.pincode} onChange={(e) => setForm({ ...form, pincode: e.target.value })} />
          </div>
        ...
        <div className={cardClass}>
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="rounded bg-slate-800 px-2 py-1 text-xs text-white transition hover:scale-105 hover:bg-slate-700"
            >
              Add Row
            </button>
          </div>
          ...
          {(form.extraServicesRows || []).map((row, index) => (
            <div key={`extra_service_${index}`} className="animate-in-row grid grid-cols-[1fr_140px_84px] gap-2">
              <input
                className={inputClass}
              ...
              <button
                ...
                className="rounded bg-rose-800 px-2 py-1 text-xs text-white transition hover:scale-105 hover:bg-rose-700"
              >
                Remove
              </button>
            </div>
        ...
        <div className={cardClass}>
          <input className={inputClass} placeholder="Govt ID URL" value={form.idProof} onChange={(e) => setForm({ ...form, idProof: e.target.value })} />
          <input type="file" accept="image/*,.pdf" onChange={(e) => uploadSingle(e.target.files?.[0], "idProof")} />
        </div>
        <div className={cardClass}>
          <input className={inputClass} placeholder="Selfie URL" value={form.selfie} onChange={(e) => setForm({ ...form, selfie: e.target.value })} />
          <input type="file" accept="image/*" onChange={(e) => uploadSingle(e.target.files?.[0], "selfie")} />
        </div>
        <div className={cardClass}>
          <input className={inputClass} placeholder="Certificate URL" value={form.certificate} onChange={(e) => setForm({ ...form, certificate: e.target.value })} />
          <input type="file" accept="image/*,.pdf" onChange={(e) => uploadSingle(e.target.files?.[0], "certificate")} />
        </div>

        <button disabled={uploading} className="rounded bg-sky-700 px-3 py-2 text-white transition duration-300 hover:-translate-y-0.5 hover:bg-sky-600 disabled:opacity-60">
          {uploading ? "Uploading..." : "Save Onboarding"}
        </button>
      </form>

      <button onClick={payFee} className="rounded bg-amber-600 px-3 py-2 text-white transition duration-300 hover:-translate-y-0.5 hover:bg-amber-500">
        Pay Verification Fee (INR 299)
      </button>
      {msg && <p className="animate-in-message text-sm text-slate-300">{msg}</p>}
      <style jsx>{`
        .onboard-shell {
          animation: shellIn 520ms cubic-bezier(0.22, 1, 0.36, 1);
        }
        .onboard-card {
        ...
        .onboard-form > * {
          ...
          animation: sectionIn 460ms cubic-bezier(0.22, 1, 0.36, 1) forwards;
        }
        .onboard-form > *:nth-child(1) { animation-delay: 40ms; }
        .onboard-form > *:nth-child(2) { animation-delay: 80ms; }
        .onboard-form > *:nth-child(3) { animation-delay: 120ms; }
        .onboard-form > *:nth-child(4) { animation-delay: 160ms; }
        .onboard-form > *:nth-child(5) { animation-delay: 200ms; }
        .onboard-form > *:nth-child(6) { animation-delay: 240ms; }
        .onboard-form > *:nth-child(7) { animation-delay: 280ms; }
        .onboard-form > *:nth-child(8) { animation-delay: 320ms; }
        .animate-in-row {
          animation: rowIn 280ms ease-out;
        }
        .animate-in-checklist {
          opacity: 0;
          animation: checklistIn 360ms ease-out forwards;
        }
        .animate-in-message {
